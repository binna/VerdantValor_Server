...
### A. 개요
본 사이드 프로젝트는    
클라이언트가 전송한 메시지를 연결된 모든 클라이언트에게 브로드캐스트하는 에코 TCP 서버입니다.    
...
클라이언트 요청을 1000회 연속으로 전송하여 RTT를를 측정했으며,    
...
1 세션 1 스레드 구조에서는 세션당 Task를 생성해 전용 스레드처럼 동작하도록 구성했습니다.   
다만 C#의 Task는 내부적으로 상태 머신으로 동작하므로,      
모든 작업이 전용 스레드를 점유한 채 대기하는 것은 아닙니다.     

<br>

이 차이는 Receive를 대기하는 방식에서 발생했습니다.     

<br>

1 세션 1 스레드 구조에서는    
각 세션에 전용으로 할당된 스레드를 깨워 작업을 수행해야 하며,     
이 과정에서 스레드 스케줄링과 컨텍스트 스위칭 비용이 반복적으로 발생합니다.    

반면 EAP 비동기 구조에서는  
I/O 완료 시점에 처리 흐름이 이어지도록 구성되어,    
대기 중인 전용 스레드를 깨우는 과정 없이    
완료된 작업을 처리할 수 있었습니다.    
