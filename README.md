...
> Windows 환경의 IOCP는    
> 비동기 I/O 완료를 큐에 전달하고    
> 미리 준비된 워커 스레드가 이를 처리하는 모델이며    
> 본 구현은 이러한 완료 중심 처리 구조의 개념을 참고했습니다.    
...
본 구현에서는 **I/O 완료 시점을 기준으로     
처리 흐름이 이어지도록 구성하여    
IOCP 스타일의 I/O 처리 구조**를 최대한 반영하고자 했습니다.     

다만 IOCP처럼 I/O 완료를    
단일 completion 큐로 전달하는 방식은 아니며,       
완료된 작업을    
이벤트 기반 비동기 처리 흐름으로 전달받아 처리합니다.    
...
IOCP 스타일 구조가 전반적으로 성능이 크게 개선된 것을 확인할 수 있었습니다.**    
...
이 차이는 Send / Receive 호출 시점에서의 처리 방식 차이에서 발생했습니다.     
...
응답 시간 분포의 안정성 차이로 이어졌다고 판단할 수 있습니다.  
