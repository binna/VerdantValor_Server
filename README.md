...
- 세션 수만큼 스레드가 필요합니다.    
  ...
  컨텍스트 스위칭 비용 증가 등의 리소스 낭비가 발생할 수 있습니다.    
...
1 세션 1 스레드 구조에서는 세션당 Task를 생성했습니다.   

<br>

1 세션 1 스레드 구조에서는    
각 세션에 전용으로 할당된 Task의 실행을 재개하기 위해     
스레드 스케줄링과 컨텍스트 스위칭 비용이 반복적으로 발생합니다.    

반면 EAP 비동기 구조에서는  
I/O 완료 시점에 처리 흐름이 이어지도록 구성되어,    
세션 단위로 대기 중인 실행 흐름을 재개하지 않고    
완료된 작업을 처리할 수 있었습니다.    
